...
browser.runtime
  ...
  .then((preference) => {
    ...
    const handleProf = () => {
      ...
      if (submitButton != null) {
        ...
        submitButton.setAttribute(
          "onclick",
          "document.form1.method = 'POST'; document.form1.action = 'rev_feed_submit.jsp'; document.form1.submit();"
        );
    ...
    const handleCourse = () => {
      course = document
        .getElementById("myframe")
        .contentDocument.querySelectorAll('a[href="javascript:void(0)"]');
    ...
    try {
      if (!preference.all) fill_form();
      else if (courseCounter == 0) handleCourse();
    } catch (err) {
      console.error(err);
    }
  });
